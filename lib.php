 ...
 */
function local_mlangremover_extend_navigation_course($navigation, $course) {
    ...
    // Build a moodle url.
    $url = new moodle_url("/local/mlangremover/remove_mlangs.php?courseid=$course->id&lang=$lang");

    // Get title of translate page for navigation menu.
    $title = get_string('pluginname', 'local_mlangremover');

    // Navigation node.
    $translatecontent = navigation_node::create($title, $url, navigation_node::TYPE_CUSTOM, $title, 'remove_mlangs',
            new pix_icon('icon', 'icon', 'local_mlangremover'));
    // Do not show in menu if no capability.
