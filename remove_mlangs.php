...
$currentlang = optional_param('lang', 'en', PARAM_TEXT);
try {
...
$PAGE->set_url('/local/mlangremover/remove_mlangs.php', ['course_id' => $courseid]);
...
$PAGE->requires->js_call_amd('local_mlangremover/mlangremover', 'init', [$jsconfig]);

$output = $PAGE->get_renderer('local_mlangremover');
...
$coursedata = new \local_mlangremover\local\data\course_data($course, $currentlang, $context->id);
// Build the page.
$prepareddata = $coursedata->getdata();

$renderable = new \local_mlangremover\output\remove_mlangs_page($course, $prepareddata, $mlangfilter, $coursedata->get_mlangtags(),
        $plugin->release);
